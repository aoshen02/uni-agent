    ...
    async def _start(self):  # type: ignore[override]
        async with mod._get_starting_semaphore():
            ...
            self._max_concurrent_observed = max(
                self._max_concurrent_observed, self._concurrent_in_start
            )
            try:
...
def test_start_wall_budget_aborts_before_attempt_when_exhausted(monkeypatch):
    ...
    assert dep._start_calls == 1, (
        f"second attempt must be skipped after wall budget exhausted, "
        f"got start_calls={dep._start_calls}"
    )
...
def test_starting_semaphore_caps_global_in_flight(monkeypatch):
    ...
    assert shared["peak"] <= 2, (
        f"semaphore must cap concurrent _start bodies at 2 (= 8 global / 4 workers), "
        f"observed peak={shared['peak']}"
    )
...
def test_starting_semaphore_released_on_failure(monkeypatch):
    ...
    dep_b = _make_deployment(
        start_fn=lambda d: _ok()
    )  # must succeed -- permit must be free
