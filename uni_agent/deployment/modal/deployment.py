...
class ModalDeployment(AbstractDeployment):
    ...
    async def start(self, max_retries: int = 2):
        ...
        for retry in range(max_retries):
            ...
            if remaining <= 0:
                self.logger.critical(
                    f"Wall-clock budget {_INIT_WALL_BUDGET}s exhausted before attempt {retry + 1}"
                )
                break
            try:
        ...
        raise RuntimeError(
            f"Failed to create modal sandbox after {max_retries} retries "
            f"(wall budget {_INIT_WALL_BUDGET}s)"
        ) from last_error
    ...
    async def stop(self):
        ...
        if self._runtime is not None:
            try:
                ...
                self.logger.warning(
                    f"runtime.close() swallowed (continuing teardown): "
        ...
        if self._sandbox is not None:
            try:
                ...
                try:
                    ...
                    self.logger.error(
                        f"sandbox.terminate.aio() retry also failed: "
                        f"{type(exc2).__name__}: {exc2}. Sandbox may leak."
                    )
            self._sandbox = None
